...
import Footer from "./NavBar/Footer"; // ✅ Import your Footer here
...
import PrivacyPolicy from "./Components/IT22101488/PrivacyPolicy";
...
function App() {
  ...
  const noNavPaths = ["/", "/login", "/register", "/forgotpassword"];
  const hideNavOrFooter =
    noNavPaths.includes(pathname) || pathname.startsWith("/reset_password");

  return (
    <div className="App">
      {!hideNavOrFooter && <Navbar />}
      ...
      {!hideNavOrFooter && <Footer />} {/* ✅ Conditionally render Footer too */}
    </div>
  );
}
